...
def executar_ocr_captcha():
    ...
    try:
        ...
        # Convertendo imagem para escala de cinza e OCR
        captcha_text = pytesseract.image_to_string(image, config='--psm 8 digits')
        captcha_text = ''.join(filter(str.isdigit, captcha_text))[:4]

        if captcha_text and len(captcha_text) == 4:
            input_box = driver.find_element(By.NAME, "codigoCaptcha")
            input_box.clear()
            ...
            raise ValueError("OCR não conseguiu identificar 4 dígitos")
    except Exception as e:
        registrar_log(nome, f"Erro OCR: {e}", "")
...
╚═╝      ╚═════╝ ╚═╝  ╚═╝╚═╝     ╚═╝    ╚══════╝╚═╝  ╚═╝ ╚═════╝   ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝   ╚═╝    ╚═════╝ ╚═╝  ╚═╝
"""
label_ascii = tk.Label(root, text=ascii_art, font=("Courier", 8), fg="#FFA500", bg="#1e1e1e", justify="left")
